...
final class TransformedWorkflowCache
{
     ...
     *
     * @param workflowCachePeriodAsStr an ISO-8601 time duration string. If passed as {@code null} or empty a default value of 5 minutes
     * will be used.
     ...
     */
    public TransformedWorkflowCache(
        final String workflowCachePeriodAsStr,
    ...
    /**
     * Retrieves the result of a workflow transformation using combination of the provided workflow ID, project ID and partial storage
     * reference as a key. If no entry is found for the key then an attempt will be made to transform the workflow with that result being
     * returned.
     *
     ...
     * @throws ApiException if certain failures occur communicating with the processing service to retrieve the workflow during load of
     * workflow for transformation. e.g. Invalid requests will result in this exception.
     ...
     * during load of workflow for transformation. e.g. The processing service not being contactable.
     * @throws DataStoreException if a failure occurs storing a transformed workflow during load.
     * @throws UncheckedExecutionException if an unchecked exception occurs during load of transformed workflow result that is not
    ...
    /**
     * Retrieves a workflow referenced in @{code extractedProperties} and creates a script representing that workflow, storing it in the
     * data store.
     *
